// Validation failure in deeply nested structure (10 levels deep)
// Tests error detection and reporting at depth

function createValidDeepObject(depth = 10, index = 0) {
  if (depth === 0) {
    return {
      id: index,
  ...
  return {
    id: index,
    level: depth,
    nested: createValidDeepObject(depth - 1, index + 1),
  }
}

function createInvalidDeepObject(depth = 10, index = 0) {
  if (depth === 0) {
    return {
      id: 'invalid-number', // Invalid: should be number
      value: Math.random() * 1000,
      name: `level-${index}`,
    }
  }
  return {
    id: index,
    level: depth,
    nested: createInvalidDeepObject(depth - 1, index + 1),
  }
}
...
export const testDataObj = {
  data: Array.from({ length: 50 }, (_, i) => {
    if (i === 25) {
      return createInvalidDeepObject(10, i)
    }
    return createValidDeepObject(10, i)
  }),
}
